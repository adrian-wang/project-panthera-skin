...
import com.intel.ssg.dcst.panthera.parse.sql.transformer.QueryInfo;
import com.intel.ssg.dcst.panthera.parse.sql.transformer.QueryInfo.Column;
...
public class FilterBlockUtil {
   ...
   * <li> for count(*) or count(integer), will be removed from select list and restored in countAsterisk.
   * for count(col) and other aggregation funcs will be restored in aggregationList.
   *
   * @param selectList
  ...
  static List<CommonTree> filterAggregation(CommonTree selectList, CountAsterisk countAsterisk, CommonTree order) {
    ...
    for (int i = 0; i < selectList.getChildCount(); i++) {
      ...
      // TODO only one function supported now, nested function is not supported, e.g. avg(max(col)).
      ...
      if (standardFunctionList.size() == 1) {
        ...
        // count(*/1) will be saved in class countAsterisk.
        ...
        if (standardFunction.getChild(0).getType() == PantheraParser_PLSQLParser.COUNT_VK
          ...
          // the count(*)/count(1) are restored in coutAsterisk.selectItem.
          // the select Item will be deleted later.
        ...
        // count(col) and other aggregation funcs will be saved in aggregationList.
        List<CommonTree> exprList = new ArrayList<CommonTree>();
        findNode(standardFunction, PantheraParser_PLSQLParser.EXPR, exprList);
        // FIXME: here can not handle conditions like: round(max(col1), 2)
    ...
    // delete select item contains count(*) or count(1)
    // if select list only have one select item, there, only remains a single selectList node.
    // for this single selectList node, will be handled in BaseFilterBlockProcessor.process().
  ...
  /**
   * find all node which type is input type in the tree which root is node.
   * and not separated by SELECT_STATEMENT node
   *
   * @param node
   * @param type
   * @param nodeList
   */
  public static void findNodeInCurrentLayer(CommonTree node, int type, List<CommonTree> nodeList) {
    if (node == null) {
      return;
    }
    if (node.getType() == type) {
    ...
    for (int i = 0; i < node.getChildCount(); i++) {
      ...
      if (child.getType() == PantheraParser_PLSQLParser.SELECT_STATEMENT) {
        continue;
      }
      findNode(child, type, nodeList);
    }
  }
  ...
  public static void rebuildSelectAlias(CommonTree selectList) {
    for (int i = 0; i < selectList.getChildCount(); i++) {
      ...
      if (selectItem.getChildCount() == 2) {
        ...
        // TODO not use this way (use setText), just create a new select list
  ...
  /**
   * remove table node under ANY_ELEMENT node, only leave the column name.
   * @param selectList
   */
  public static void removeTableNameFromSelectList(CommonTree selectList) {
    for (int i = 0; i < selectList.getChildCount(); i++) {
      ...
      FilterBlockUtil.findNode((CommonTree) selectList.getChild(i),
