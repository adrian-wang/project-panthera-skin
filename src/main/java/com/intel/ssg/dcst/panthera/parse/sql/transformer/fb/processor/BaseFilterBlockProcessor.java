...
import com.intel.ssg.dcst.panthera.parse.sql.transformer.fb.FilterBlock;
import com.intel.ssg.dcst.panthera.parse.sql.transformer.fb.FilterBlockContext;
...
import com.intel.ssg.dcst.panthera.parse.sql.transformer.fb.QueryBlock;
import com.intel.ssg.dcst.panthera.parse.sql.transformer.fb.SubQFilterBlock;
...
public abstract class BaseFilterBlockProcessor implements FilterBlockProcessor {
  ...
  /**
   * add column for top query which is select count(*) or count(1) or *
   *
   * @throws SqlXlateException
   *
   */
    ...
    if (columnList != null && columnList.size() > 0) {
      if (selectList == null) {
        if (topSelect.getFirstChildWithType(PantheraExpParser.ASTERISK) != null) {
          // which is *, will replace * with selectList node,
      ...
      int count = 0;
      // for single count(*), count(integer), * in select list, will be replaced with all columns of the table.
  ...
  /**
   * create select list branch, the branch is count(*)
   *
   * @return
   *    return the selectList node. The selectList node has only one selectItem which is count(*).
   */
  ...
  /**
   * for NOT EXISTS uncorrelated, will use cross join and add a filter to the where branch under closing select.
   * the filter is : Output row of the bottom select is 0.
   *
   * @param not
   * @param select
   * @param tabAlias
   ...
   * @return
   */
  CommonTree buildWhereForAll(CommonTree leftOp) {
  ...
  /**
   * only remain table name under ANY_ELEMENT, table name node will be deleted
   *
   * @param cascated
   */
